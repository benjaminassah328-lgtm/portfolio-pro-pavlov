export const profile = {
  name: "Benjamin Assah",
  title: "Développeur Web",
  tagline:
    "Passionné par le code, curieux par nature — fraîchement diplômé et prêt à construire.",
  email: "benjaminassah328@gmail.com",
  github: "https://benjaminassah328-lgtm",
  linkedin: "https://linkedin.com/in/benjaminassah",
  cvPath: "/cv-benjamin-assah.pdf",
};

export const about = {
  text: "Fraîchement diplômé, je suis un développeur généraliste passionné par la création d'expériences web. J'aime apprendre, construire des choses utiles et relever de nouveaux défis technologiques. Mon parcours m'a permis de toucher à de nombreux domaines du développement — front, back, bases de données — et je suis à la recherche de ma première opportunité professionnelle pour continuer à grandir.",
};

export const education = [
  {
    id: 1,
    school: "UIPA",
    degree: "BTS en Informatique Developpeur d'Application (IDA)",
    year: "2024 – 2026",
    description:
      "Formation générale en développement logiciel, algorithmique, bases de données et systèmes d'information.",
  },
  {
    id: 1,
    school: "Lunion-Lab",
    degree: "Developpeur Junior",
    year: "2025",
    description:
      "Formation générale en développement Full-stack (React, Tailwind css)",
  },
];

export const projects = [
  {
    id: 1,
    title: "Home Digital",
    description:
      "Application web de vente en ligne avec gestion des produits, panier d'achat et système de commandes.",
    stack: ["React", "Tailwind CSS"],
    github: "https://home-digital-eul3.vercel.app",
    type: "Personnel",
  },
  {
    id: 2,
    title: "lagague-music",
    description:
      "Site de Music conçu pour la promotion des jeunes talents d'afrique.",
    stack: ["React", "Tailwind CSS"],
    github: "https://lagague-music.vercel.app",
    type: "Personnel",
  },
  {
    id: 3,
    title: "Restaurant Pavlov",
    description:
      "Site de restaurant pour recevoir les commandes via whatsApp",
    stack: ["React", "Tailwind CSS"],
    github: "https://restaurant-pavlov-6b3fbscsk-benjaminassah328-lgtms-projects.vercel.app",
    type: "Personnel",
  },

  {
    id: 4,
    title: "Dri valé",
    description:
      "Site de restaurant pour recevoir les commandes via whatsApp",
    stack: ["React", "Tailwind CSS"],
    github: "https://www.dri-vale.online",
    type: "Dri valé",
  },
];
